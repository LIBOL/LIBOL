function [model, hat_y_t, l_t] = M_aROMMA(y_t, x_t, model)
% M_ROMMA: Multi-class aggressive ROMMA algorithms.
%--------------------------------------------------------------------------
%Reference:
%Yi Li, Philip M. Long: The Relaxed Online Maximum Margin Algorithm. Machine Learning 46(1-3): 361-387 (2002)
%--------------------------------------------------------------------------
% INPUT:
...
% Making Update
%--------------------------------------------------------------------------
l_t = max(0, 1 - (F_t(y_t) - F_t(s_t)));  % hinge loss
if (l_t > 0),
    if (norm_W == 0),
