function [model, hat_y_t, l_t] = M_SCW2(y_t, x_t, model)
% M_SCW2: Soft Multiclass Confidence Weight Learning algorithm (M-SCW-II)
%--------------------------------------------------------------------------
% Reference:
...
%--------------------------------------------------------------------------
if m_t < phi*sqrt(v_t)
    alpha_t = max(0,(-(2*m_t*n_t+phi^2*m_t*v_t)+sqrt(phi^4*m_t^2*v_t^2+8*n_t*v_t*phi^2*(n_t+v_t*phi^2)))/(2*(n_t^2+n_t*v_t*phi^2)));
