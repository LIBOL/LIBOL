	...
	if (l_t > 0)
	{
        ...
        if (norm_E > 0){
            ...
            for (int i = 0; i< norm_E; ++i){
                int E_i = E[i] - 1;
                sum_F_t_E = sum_F_t_E + F_t[E_i];
            }
            double denominator = sum_F_t_E - norm_E*F_t[Y-1];
            for (int i = 0; i< norm_E; ++i){
                int s_t = E[i];
                double numerator = F_t[s_t-1] - F_t[Y-1];
                times_scale(X, -(numerator/denominator), D);
                memcpy(W+(s_t-1)*D,v_plus(W+(s_t-1)*D, X, D),D*sizeof(double));
                times_scale(X, -(denominator/numerator), D);
            }
        }
