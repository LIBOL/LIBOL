...
void mexFunction(int nlhs, mxArray *plhs[],
    ...
    mxArray* r_ptr = mxGetField(prhs[2],0,"r");
    if (!r_ptr) mexErrMsgTxt("model.r is null");
	double r = mxGetScalar(r_ptr);
	...
	if (l_t > 0)
	{
        double beta_t = 1/(v_t + r);
        double alpha_t = l_t*beta_t;
	...
	const char *field_names[] = {"W","nb_class","Sigma","r"};
    ...
    mxArray* r_mxarray = mxCreateDoubleMatrix(1,1,mxREAL);
    double* r_ptr_mxarray = mxGetPr(r_mxarray);
    r_ptr_mxarray[0] = r;
    mxSetField(plhs[0], 0, "r", r_mxarray);
