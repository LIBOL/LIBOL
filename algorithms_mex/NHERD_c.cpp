...
void mexFunction(int nlhs, mxArray *plhs[],
    ...
    mxArray* gamma_ptr = mxGetField(prhs[2],0,"gamma");
    if (!gamma_ptr) mexErrMsgTxt("model.gamma is null");
	double gamma = mxGetScalar(gamma_ptr);
	...
	if (l_t > 0)
	{
        double beta_t  = 1/(v_t + gamma);
        double alpha_t = max(0,1-m_t)*beta_t;
        for (int i = 0; i < D; i++)
            for (int j = 0; j < D; j++)
                Sigma[i*D+j] = Sigma[i*D+j] - pow(beta_t,2)*(v_t+2*gamma)*v_S_x_t[i]*v_S_x_t[j];        
	...
	const char *field_names[] = {"w","Sigma","gamma"};
    ...
    mxArray* gamma_mxarray = mxCreateDoubleMatrix(1,1,mxREAL);
    double* gamma_ptr_mxarray = mxGetPr(gamma_mxarray);
    gamma_ptr_mxarray[0] = gamma;
    mxSetField(plhs[0], 0, "gamma", gamma_mxarray);
