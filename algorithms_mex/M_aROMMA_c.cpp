...
void mexFunction(int nlhs, mxArray *plhs[],
    ...
    l_t = max(0, 1 - (F_t[Y-1] - F_t[s_t-1])); 
    
	if (l_t > 0)
	{   
        double norm_W = v_times(W, W, D*nb_class);
        double norm_x_t = v_times(X, X, D); 
        if (norm_W == 0)
        {
        ...
        {   
            double c_t = (2*norm_x_t*norm_W - (F_t[Y-1] - F_t[s_t-1]))/(2*norm_x_t*norm_W - (F_t[Y-1] - F_t[s_t-1])*(F_t[Y-1] - F_t[s_t-1]));
            double d_t = (norm_W*(1 - (F_t[Y-1] - F_t[s_t-1])))/(2*norm_x_t*norm_W - (F_t[Y-1] - F_t[s_t-1])*(F_t[Y-1] - F_t[s_t-1]));
            times_scale(W+(Y-1)*D, c_t, D);
            times_scale(W+(s_t-1)*D, c_t, D);
            times_scale(X, d_t, D);        
	        memcpy(W+(Y-1)*D,v_plus(W+(Y-1)*D, X, D),D*sizeof(double));
            times_scale(X, -1, D);        
            memcpy(W+(s_t-1)*D,v_plus(W+(s_t-1)*D, X, D),D*sizeof(double));
