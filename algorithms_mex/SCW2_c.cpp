...
void mexFunction(int nlhs, mxArray *plhs[],
    ...
    double n_t = v_t + 1/(2*C);
    double m_t = Y*f_t;
    l_t = phi*sqrt(v_t)-m_t;

	if (l_t > 0)
	{
	    double alpha_t = max(0,(-(2*m_t*n_t+pow(phi,2)*m_t*v_t) + sqrt(pow(phi,4)*pow(m_t,2)*v_t*2+4*n_t*v_t*pow(phi,2)*(n_t+v_t*phi*2)))/(2*(pow(n_t,2)+n_t*v_t*pow(phi,2))));
